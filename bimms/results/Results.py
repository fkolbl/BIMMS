...
class bode_results(BIMMS_results):
    ...
    def update(self, __m, **kwargs) -> None:
        if isinstance(__m, bode_results):
            ...
            self['mag_ch1_raw'] = np.concatenate([self['mag_ch1_raw'], __m['mag_ch1_raw']])
            self['mag_ch2_raw'] = np.concatenate([self['mag_ch2_raw'], __m['mag_ch2_raw']])
